#ifndef CROSSCOMPONENTITERATOR_H_
#define CROSSCOMPONENTITERATOR_H_
...
class CrossComponentIterator :  AbstractGraphIterator<V, E>
//JAVACODE: public abstract class CrossComponentIterator<V, E, D> extends AbstractGraphIterator<V, E>
{
    //~ Static fields/initializers ---------------------------------------------

private:
	static const int CCS_BEFORE_COMPONENT = 1;
	//JAVACODE:   private static final int CCS_BEFORE_COMPONENT = 1;
    static const int CCS_WITHIN_COMPONENT = 2;
    static const int CCS_AFTER_COMPONENT = 3;

    //~ Enums ------------------------------------------------------------------

    //Standard vertex visit state enumeration.
protected:
    static enum VisitColor
    {
    ...
    //
private:
    const ConnectedComponentTraversalEvent ccFinishedEvent =
        new ConnectedComponentTraversalEvent(
            this,
            ConnectedComponentTraversalEvent.CONNECTED_COMPONENT_FINISHED);
    const ConnectedComponentTraversalEvent ccStartedEvent =
        new ConnectedComponentTraversalEvent(
            this,
    ...
    // during iteration.
    FlyweightEdgeEvent<V, E> reusableEdgeEvent;
    FlyweightVertexEvent<V> reusableVertexEvent;
    Iterator<V> vertexIterator = null;
    ...
    int state = CCS_BEFORE_COMPONENT;
...
public:
     ...
     */
protected:
    virtual boolean isConnectedComponentExhausted();
     ...
     */
    virtual void encounterVertex(V vertex, E edge);
     ...
     */
    virtual void encounterVertexAgain(V vertex, E edge);
...
private:
    void addUnseenChildrenOf(V vertex)
    {
    ...
    EdgeTraversalEvent<V, E> createEdgeTraversalEvent(E edge)
    {
        if (isReuseEvents()) {
    ...
    VertexTraversalEvent<V> createVertexTraversalEvent(V vertex)
    {
        if (isReuseEvents()) {
    ...
    static class  SimpleContainer<T> /*interface virtual und =0; */
    {
        /**
...
//     }
   ...
   static class Specifics
    {
public:
    	virtual set<EE*>* edgesOf(VV vertex)= 0;
    };
     ...
     * @since Aug 11, 2003
     */
    template <class VV, class localE>
   static class FlyweightEdgeEvent : EdgeTraversalEvent<VV, localE>
    {
private:
	   static const long serialVersionUID = 4051327833765000755L;

        /**
         * @see EdgeTraversalEvent#EdgeTraversalEvent(Object, Edge)
         */
public:
        FlyweightEdgeEvent(Object eventSource, localE edge)
        {
...
protected:
     ...
     */
   template <class VV>
    static class FlyweightVertexEvent : VertexTraversalEvent<VV>
    {
private:
    	static const long serialVersionUID = 3834024753848399924L;

        /**
         * @see VertexTraversalEvent#VertexTraversalEvent(Object, Object)
         */
public:
    	FlyweightVertexEvent(Object eventSource, VV vertex)
        {
...
private:
    template <class VV, class EE>
    static class DirectedSpecifics : Specifics<VV, EE>
    {
private:
    	DirectedGraph<VV, EE> graph;
         ...
         */
public:
    	DirectedSpecifics(DirectedGraph<VV, EE> g)
        {
            graph = g;
        }
...
private:
    template <class VV, class EE>
    static class UndirectedSpecifics<VV, EE> :  Specifics<VV, EE>
    {
private:
    	Graph<VV, EE> graph;
...
#endif /* CROSSCOMPONENTITERATOR_H_ */
